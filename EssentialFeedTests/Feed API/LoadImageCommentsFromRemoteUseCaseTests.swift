...
//  EssentialFeedTests
//
//  Created by Marian Stanciulica on 12.01.2023.
//
...
class LoadImageCommentsFromRemoteUseCaseTests: XCTestCase {
    ...
    func test_load_deliversErrorOn200HTTPResponseWithInvalidJSON() {
        ...
        expect(sut, toCompleteWithResult: failure(.invalidData)) {
            let invalidJSON = Data(bytes: "invalid json", count: 10)
    ...
    private func expect(_ sut: RemoteImageCommentsLoader, toCompleteWithResult expectedResult: RemoteImageCommentsLoader.Result, when action: () -> Void, file: StaticString = #filePath, line: UInt = #line) {
        ...
        sut.load() { receivedResult in
            ...
            case let (.success(receivedIems), .success(expectedItems)):
                XCTAssertEqual(receivedIems, expectedItems, file: file, line: line)
